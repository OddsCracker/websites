...
class Ajax extends MY_Controller {
	...
	public function check_login_status(){
		if (!get_cookie('_user_')) {
			$res['status'] = 'error';
			$res['msg'] = 'You need to login to access this resource.';
	...
	public function index(){
		$res['status'] = 'success';
		$res['msg'] = 'Nothing to do here, thank you for your interest though.';
		header('Content-type:application/json');
		echo json_encode($res);
	}
	...
	public function register(){
		...
		if (!$errors) {
			$res['status'] = 'success';
			$res['msg'] = 'Thank you for registering<br>We sent an activation email to your email address.<br>Please check Spam/Junk folders as well since it might and up there.<br>Use the email instructions to activate your account.';
		} else {
			$res['status'] = 'error';
			$res['msg'] = $errors;
		}
	...
	public function login(){
		...
		if (!$errors) {
			$res['status'] = 'success';
			$res['msg'] = 'Login successful, redirecting you to your profile page ...';
		} else {
			$res['status'] = 'error';
			$res['msg'] = $errors;
		}
	...
	public function contact(){
		...
		if (!$errors) {
			$res['status'] = 'success';
			$res['msg'] = 'Thank you for your message, we will get back you!';
		} else {
			$res['status'] = 'error';
			$res['msg'] = $errors;
		}
	...
	public function rate(){
		...
		if (!$rating) {
			$res['status'] = 'error';
			$res['msg'] = 'IP address already rated!';
		} else {
			$res['status'] = 'success';
			$res['msg'] = $rating;
		}
	...
	public function subscribe(){
		...
		if (!$subscribed) {
			$res['status'] = 'error';
			$res['msg'] = 'You already subscribed to our newsletter!';
		} else {
			$res['status'] = 'success';
			$res['msg'] = 'Thank you for your subscription!';
		}
		header('Content-type:application/json');
		echo json_encode($res);
	}
	...
	public function facebook_login(){
		...
		if (!$errors) {
			$res['status'] = 'success';
			$res['msg'] = 'You logged in succesfully using your Facebook account!';
		} else {
			$res['status'] = 'error';
			$res['msg'] = $errors;
		}
	...
	public function google_login(){
		...
		if (!$errors) {
			$res['status'] = 'success';
			$res['msg'] = 'You logged in succesfully using your Google account!';
		} else {
			$res['status'] = 'error';
			$res['msg'] = $errors;
		}
	...
	public function get_odds($event_id){
		...
		$market_labels = array('1x2'=>'1x2 odds', 'dc'=>'Double Chance', 'OU'=>'Over/Under 2.5');
		...
		if (!$odds) {
			echo '
			<h3 class="text-center">Odds still not available!</h3>
			<h4 class="green-bg"><button class="btn btn-warning" data-toggle="modal" data-target="#newsletter_modal">Subscribe</button> to get full functionality at <strong>' . $this->data['settings']['site_name'] . '</strong></h4>
			';
		} else {
			echo '
			<div class="odds-tab">
...
/*************************** bets with at most 3 options (1x2, double chance, over/under, both teams to score, etc.) **********************************/
						foreach ($odds['markets'][$market] as $bookmaker => $bet) {
							if ($bookies[$bookmaker]['DisplayOdds'] == 0 && !get_cookie('_user_')) {
								...
								<li class="single-row">
									...
									<div class="text-center"><a href="#" title="Login" data-toggle="modal" data-target="#login_modal">Login to display the odds!</a></div>
								</li>
								';
							} else {
								echo '
						...
						echo '
						<li class="single-row highest">
							<div class="row-title"><i class="fa fa-fw fa-signal text-success"></i> Highest</div>
							';
						...
						<li class="single-row avg">
							<div class="row-title"><i class="fa fa-fw fa-code text-success"></i> Average</div>
							';
...
/************************* bets with more than 3 options (correct score, etc.) **********************/
						foreach ($odds['stats'][$market]['high'] as $key => $value) {
									...
									foreach ($bet as $okey => $ovalue) {
										if ($okey == $key) {
											...
											<li class="single-row">
												...
												<div class="pull-right"><a href="#" title="Login" data-toggle="modal" data-target="#login_modal">Login to display the odds!</a></div>
											</li>
											';
										}
									}
							...
							echo '
								<li class="single-row highest">
									<div class="row-title"><i class="fa fa-fw fa-signal text-success"></i> Highest</div><div class="odds-cell pull-right">' . $odds['stats'][$market]['high'][$key] . '</div>
								</li>
								<li class="single-row avg">
									<div class="row-title"><i class="fa fa-fw fa-code text-success"></i> Average</div><div class="odds-cell pull-right">' . round(floatval($odds['stats'][$market]['avg'][$key]), 2) . '</div>
								</li>
							</div>
							';
						}
		...
		$end = microtime(true);
		echo '<br><small><i>Odds generated in ' . round(($end-$start), 3) . ' seconds</i></small>';
		endif;
	}
