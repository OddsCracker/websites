...
<div id="center-col" class="col-sm-6">
	...
	<div class="row text-center">
		<h2>Choose a date:</h2> <input type="text" name="datetime" class="datepicker center-block" data-date-format="dd/mm/yyyy" data-date-end-date="-1d" value="<?php echo date('d/m/Y', strtotime('-1 day'));?>" required="required" placeholder="Select date">
	</div>
	<?php
	if (count($archives) > 0) {
		?>
		<h3 class="up-next">Archived betting odds <?php if ($this->uri->segment(2)) { echo preg_replace('/[^0-9\-]/', '', $this->uri->segment(2)); } else { echo date('Y-m-d', strtotime('-1 day')); } ?></h3>
	...
	?>
		<h3 class="text-center">No archived betting odds for <?php if ($this->uri->segment(2)) { echo preg_replace('/[^0-9\-]/', '', $this->uri->segment(2)); } else { echo date('Y-m-d', strtotime('-1 day')); } ?></h3>
	<?php
	}
	?>
</div>
