...
class Cron extends MY_Controller {
	...
	public function update_news(){
		...
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, 'https://www.journalducameroun.com/feed/');
		...
		foreach ($xml->channel->item as $item){
			...
			if (strstr($categories, 'sport')) {
				$this->db->where('Url', $Url);
				$qn = $this->db->get('oc_news');
				$cn = $qn->num_rows();
				if ($cn == 0) {
					$data = array('DateTime'=>$DateTime, 'Title'=>$Title, 'Source'=>$Source, 'Url'=>$Url);
		...
		/* cameroononline.org */
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, 'https://www.cameroononline.org/category/sports-news/feed/');
		curl_setopt($ch, CURLOPT_TIMEOUT, 45);
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 30);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		$res = curl_exec($ch);
		...
		/* cameroon-today.com */
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, 'http://news.cameroon-today.com/feed/');
		curl_setopt($ch, CURLOPT_TIMEOUT, 45);
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 30);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		$res = curl_exec($ch);
		...
		/* actucameroun.com */
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, 'https://actucameroun.com/feed/');
		curl_setopt($ch, CURLOPT_TIMEOUT, 45);
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 30);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		$res = curl_exec($ch);
