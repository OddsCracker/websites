...
<div id="center-col" class="col-sm-6">
	<h1 class="up-next">
		<i class="fa fa-lg fa-angle-double-right"></i> Cotes des paris suivants
...
if (!$this->uri->segment(2)) {
	if (!$events) {
	?>
	<h2 class="text-center">Aucun événement de paris sportifs programmé</h2>	
	<?php
	} else {
	?>
	<table class="table table-striped">
		...
		?>
			<tr>
				...
				<td>
					<a href="/event/<?php echo preg_replace('/[^a-zA-Z0-9\-]/', '', str_replace(' ', '-', strtolower($value['Team1']))) . '-' . preg_replace('/[^a-zA-Z0-9\-]/', '', str_replace(' ', '-', strtolower($value['Team2']))) . '-' . $key;?>" title="<?php echo $value['Team1'];?> - <?php echo $value['Team2'];?>">
						<span class="sr-only">Obtenez les meilleures cotes de paris sportifs pour </span><?php echo $value['Team1'];?> - <?php echo $value['Team2'];?><span class="sr-only"> en <?php echo $value['LeagueName'];?></span>
					</a>
				</td>
				<td class="pull-right">
...
?>
		<h2 class="text-center">Aucun événement de paris sportifs programmé pour <?php echo preg_replace('/[^a-zA-Z0-9\-\s]/', '', str_replace('_', ' ', $this->uri->segment(2))) ?></h2>
<?php
	} else {
?>
		<table class="table table-striped">
		...
		<div class="grey-bg text-center row">
			<div class="col-sm-6">
				<a href="http://www.facebook.com/sharer.php?u=<?php echo $this->data['settings']['abs_url'];?>/events/<?php echo preg_replace('/[^a-zA-Z0-9\-\s]/', '', str_replace('_', ' ', $this->uri->segment(2)));?>" title="Share on Facebook" class="facebook-share" target="_blank"><i class="fa fa-lg fa-facebook"></i> Partager sur Facebook</a>
			</div>
			<div class="col-sm-6">
