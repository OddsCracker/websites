...
class Cron extends MY_Controller {
	...
	public function update_news(){
		...
		/* punchng.com */
 		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, 'http://punchng.com/topics/sports/feed/');
		...
		/* pulse.ng */
 		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, 'http://www.pulse.ng/sports/football/rss');
		curl_setopt($ch, CURLOPT_TIMEOUT, 45);
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 30);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		$res = curl_exec($ch);
		...
		foreach ($xml->channel->item as $item){
			...
			$Title = $item->title;
			$DateTime = date('Y-m-d H:i:s', strtotime($item->pubDate));
			$Content = strip_tags($item->children('content', true));
			$this->db->where('Url', $Url);
			$qn = $this->db->get('oc_news');
			$cn = $qn->num_rows();
			if ($cn == 0) {
		...
		/* allnigeriasoccer.com */
  		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, 'http://feeds.feedburner.com/Allnigeriasoccer?format=xml');
		curl_setopt($ch, CURLOPT_TIMEOUT, 45);
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 30);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		$res = curl_exec($ch);
		...
		/* soccernet.com.ng */
 		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, 'https://www.soccernet.com.ng/feeds/posts/default?alt=rss');
		curl_setopt($ch, CURLOPT_TIMEOUT, 45);
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 30);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		$res = curl_exec($ch);
		...
		/* vanguardngr.com */
 		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, 'https://www.vanguardngr.com/category/sports/feed/');
		curl_setopt($ch, CURLOPT_TIMEOUT, 45);
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 30);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		$res = curl_exec($ch);
