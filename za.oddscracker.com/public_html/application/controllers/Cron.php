...
class Cron extends MY_Controller {
	...
	public function update_news(){
		...
		/* sport24.co.za */
 		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, 'http://feeds.sport24.co.za/articles/Sport/Soccer/rss');
		...
		/* kickoff.com */
 		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, 'http://www.kickoff.com/rss/general-news.php');
		curl_setopt($ch, CURLOPT_TIMEOUT, 45);
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 30);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		$res = curl_exec($ch);
		...
		foreach ($xml->entry as $entry){
			$Url = $entry->link['href'];
			$Title = $entry->title;
			$DateTime = date('Y-m-d H:i:s', strtotime($entry->published));
		...
		/* sowetanlive.co.za */
 		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, 'https://www.sowetanlive.co.za/rss/?publication=sowetan-live&section=sport');
		curl_setopt($ch, CURLOPT_TIMEOUT, 45);
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 30);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		$res = curl_exec($ch);
		...
		/* goal.com */
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, 'http://www.goal.com/en-za/feeds/news');
		curl_setopt($ch, CURLOPT_TIMEOUT, 45);
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 30);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		$res = curl_exec($ch);
		...
		/* timeslive.co.za */
  		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, 'https://www.timeslive.co.za/rss/?section=sport');
		curl_setopt($ch, CURLOPT_TIMEOUT, 45);
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 30);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		$res = curl_exec($ch);
	...
	public function update_all_markets(){
		...
		foreach ($events as $EventID => $event) {
			...
			if ($list['status'] == 'success') {
				...
				if ($list['msg'] == 'Event not found!') {
					$this->db->query("DELETE FROM oc_events WHERE EventID = " . $EventID);
					$this->db->query("DELETE FROM oc_markets WHERE EventID = " . $EventID);
				}
			}
		}
