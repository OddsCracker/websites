...
class Register extends MY_Controller {
	...
	public function activate(){
		if (!$this->uri->segment(3)){
			$this->session->set_flashdata('error', 'Invalid or missing activation code!');
		} else {
			$errors = $this->FrontModel->verify_activation($this->uri->segment(3));
			if ($errors) {
				$this->session->set_flashdata('error', $errors);
			} else {
				$this->session->set_flashdata('success', 'Thank you for validating your account! The login form will appear in 3 seconds so you can login to your account.');
			}
		}
		$this->load->view('templates/head', $this->data);
		$this->load->view('templates/left_col');;
